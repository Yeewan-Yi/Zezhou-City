...
const busRoute401 = {
  ...
  type: "江洲区区内公交线路",
  outbound: [
    ["洲角广场", "shared"], ["泽州九中", "shared"], ["江洲新城", "shared", [2, 5]],
    ["江洲工业园", "shared"], ["工气石化", "shared", [2]], ["工气园北", "shared"],
    ["白塔山", "shared", [2]], ["北庭", "shared", [2]], ["汉密佳苑", "shared"], ["江洲北站", "shared", [2, 4]]
  ],
  inbound: [
    ["江洲北站", "shared", [2, 4]], ["汉密佳苑", "shared"], ["北庭", "shared", [2]],
    ["白塔山", "shared", [2]], ["工气园北", "shared"], ["工气石化", "shared", [2]],
    ["江洲工业园", "shared"], ["江洲新城", "shared", [2, 5]], ["泽州九中", "shared"], ["洲角广场", "shared"]
  ]
};
...
app.addEventListener("mouseout", (event) => {
  const frame = event.target.closest(".bus-local-map-frame");
  if (frame && !frame.contains(event.relatedTarget)) {
    frame.querySelector(".bus-map-popup")?.classList.remove("visible");
  }
});
...
function busLocalMap401Template() {
  ...
  const outboundPaths = [
    "M671.5,510.4l3.8-2.3,1.6-.9,2.5-1.4,2.5-1.4,1.6-.9,1.7-.9,2.6-1.4,2.6-1.4,1.7-.9,1.8-.9,2.7-1.3,2.7-1.3,1.8-.9,4.4-2.1",
    "M705.5,492.2l4.5-2.1,1.4-.6,2.1-.9,2.1-.9,1.4-.6,2.4-1,2.6-1,2.6-1,2.4-.9,2-.4,2.2-.3,2.2-.2h2.1c0,0,4.3,0,4.3,0",
    "M739.9,482.3h4.3c0,.1,1.8.3,1.8.3l1.9.2,1.9.2,1.8.3,1.8.3,1.9.4,1.9.5,1.7.5,1.7.6,1.8.7,1.8.7,1.6.7,1.7.7,2.5,1.1,2.5,1.1,1.7.7,8.3,3.7,2,.8,2.2.8,2.2.8,2.1.7,2.1.6,2.3.5,2.3.5,2.1.4,2.1.3,2.3.2h2.3c0,.1,2.2.2,2.2.2h4.5",
    "M813.2,500.7l4.5-.4,2.1-.3,2.3-.4,2.3-.5,2.1-.5,2.1-.6,2.2-.7,2.2-.8,2-.8,1.9-.8,2.9-1.3,2.9-1.3,1.9-.9,1.9-.9,2.9-1.4,2.9-1.4,1.9-.9,1.9-1,2.8-1.5,2.8-1.5,1.8-1,1.8-1,2.7-1.6,2.7-1.6,1.8-1,1.7-1,2.6-1.6,2.6-1.6,1.7-1.1,4-2.7",
    "M885,468.4l3.9-2.7,1.9-1.4,2-1.6,1.9-1.7,1.8-1.6,1.7-1.7,1.8-1.9,1.8-2,1.6-1.9,1.6-1.9,1.6-2.1,1.6-2.2,1.4-2.1,1.3-2.1,1.4-2.3,1.3-2.3,1.2-2.2,2.1-4.6",
    "M916.9,430l1.9-4.7,3.5-9.6.6-1.8,1-2.7,1-2.7.6-1.8.7-1.9,1-2.9,1-2.9.7-1.9.7-1.9,1-2.9,1-2.9.7-1.9,1.6-4.4",
    "M934,383.1l1.6-4.4,6.1-8.7.6-1.9.9-2.9.8-2.9.5-1.9.5-2,.8-3.1.8-3.1.5-2,.4-1.9.6-2.9.6-2.9.4-1.9.3-2,.3-2.2.2-2.2v-2c0,0,0-2,0-2l-.2-2.1-.2-2.1-.3-1.9-.4-1.9-.6-2-.6-2-.7-1.8-.8-1.7-.9-1.8-1-1.8-1-1.6-1.1-1.6-1.3-1.6-1.3-1.6-1.3-1.4-1.4-1.4-1.6-1.4-1.7-1.3-1.6-1.1-1.7-1.1-1.9-1.1-2-1-1.9-.9,1.5-4.4",
    "M925.8,289.4l1.3-4.5.5-2.1.4-2.2.4-2.3.2-2.1-1.9-.2-2.8-.2-2.8-.2h-1.9c0-.1-1.8-.2-1.8-.2h-2.7c0-.1-2.7-.2-2.7-.2h-1.8s-2.1,0-2.1,0h-2.2s-2.2,0-2.2,0h-2.1c0,0-2.2.2-2.2.2l-2.4.2-2.4.2-2.2.2-4.8.6",
    "M887.6,276.5l-4.8.7-2.4.4-2.5.5-2.5.5-2.3.5-2.4.6-2.6.6-2.5.7-2.4.7-2.3.7-2.5.8-2.5.8-2.3.8-2,.7-2.1.8-2.1.8-2,.8-1,.4-1.5.6-1.5.7-1,.4-1,.5-1.5.7-1.5.7-1,.5,1.6,1.8,1.7,2,1.6,2,1.5,1.9,1.4,1.9,1.5,2.1,1.4,2.2,1.2,2"
  ];
  const inboundPaths = [
    "M851,306.1l-3.7,2.1-1.2-2-1.4-2.2-1.5-2.1-1.4-1.9-1.5-1.9-1.6-2-1.7-2-1.6-1.8,1-.5,1.5-.7,1.5-.7,1-.5,1-.4,1.5-.7,1.5-.6,1-.4,2-.8,2.1-.8,2.1-.8,2-.7,2.3-.8,2.5-.8,2.5-.8,2.3-.7,2.4-.7,2.5-.7,2.6-.6,2.4-.6,2.3-.5,2.5-.5,2.5-.5,2.4-.4,4.8-.7",
    "M887.6,276.5l4.8-.6,2.2-.2,2.4-.2,2.4-.2h2.2c0,0,2.1-.2,2.1-.2h2.2s2.2,0,2.2,0h2.1s1.8,0,1.8,0h2.7c0,0,2.7.2,2.7.2h1.8c0,0,1.9.2,1.9.2l2.8.2,2.8.2,1.9.2-.2,2.1-.4,2.3-.4,2.2-.5,2.1-1.3,4.5",
    "M925.8,289.4l-1.5,4.4,1.9.9,2,1,1.9,1.1,1.7,1.1,1.6,1.1,1.7,1.3,1.6,1.4,1.4,1.4,1.3,1.4,1.3,1.6,1.3,1.6,1.1,1.6,1,1.6,1,1.8.9,1.8.8,1.7.7,1.8.6,2,.6,2,.4,1.9.3,1.9.2,2.1.2,2.1v2c0,0,0,2,0,2l-.2,2.2-.3,2.2-.3,2-.4,1.9-.6,2.9-.6,2.9-.4,1.9-.5,2-.8,3.1-.8,3.1-.5,2-.5,1.9-.8,2.9-.9,2.9-.6,1.9-6.1,8.7-1.6,4.4",
    "M934,383.1l-1.6,4.4-.7,1.9-1,2.9-1,2.9-.7,1.9-.7,1.9-1,2.9-1,2.9-.7,1.9-.6,1.8-1,2.7-1,2.7-.6,1.8-3.5,9.6-1.9,4.7",
    "M916.9,430l-2.1,4.6-1.2,2.2-1.3,2.3-1.4,2.3-1.3,2.1-1.4,2.1-1.6,2.2-1.6,2.1-1.6,1.9-1.6,1.9-1.8,2-1.8,1.9-1.7,1.7-1.8,1.6-1.9,1.7-2,1.6-1.9,1.4-3.9,2.7",
    "M885,468.4l-4,2.7-1.7,1.1-2.6,1.6-2.6,1.6-1.7,1-1.8,1-2.7,1.6-2.7,1.6-1.8,1-1.8,1-2.8,1.5-2.8,1.5-1.9,1-1.9.9-2.9,1.4-2.9,1.4-1.9.9-1.9.9-2.9,1.3-2.9,1.3-1.9.8-2,.8-2.2.8-2.2.7-2.1.6-2.1.5-2.3.5-2.3.4-2.1.3-4.5.4",
    "M813.2,500.7h-4.5c0,.1-2.2,0-2.2,0h-2.3c0-.1-2.3-.4-2.3-.4l-2.1-.3-2.1-.4-2.3-.5-2.3-.5-2.1-.6-2.1-.7-2.2-.8-2.2-.8-2-.8-8.3-3.7-1.7-.7-2.5-1.1-2.5-1.1-1.7-.7-1.6-.7-1.8-.7-1.8-.7-1.7-.6-1.7-.5-1.9-.5-1.9-.4-1.8-.3-1.8-.3-1.9-.2-1.9-.2h-1.8c0-.1-4.3-.3-4.3-.3",
    "M739.9,482.3h-4.3c0,0-2.1,0-2.1,0l-2.2.2-2.2.3-2,.4-2.4.9-2.6,1-2.6,1-2.4,1-1.4.6-2.1.9-2.1.9-1.4.6-4.5,2.1",
    "M705.5,492.2l-4.4,2.1-1.8.9-2.7,1.3-2.7,1.3-1.8.9-1.7.9-2.6,1.4-2.6,1.4-1.7.9-1.6.9-2.5,1.4-2.5,1.4-1.6.9-3.8,2.3"
  ];
  return `
    <figure class="bus-local-map-card bus-local-map-card-401">
      ...
      <div class="bus-local-map-frame" style="--map-aspect:330 / 290">
        <svg viewBox="640 250 330 290" role="img" aria-labelledby="bus-401-map-title bus-401-map-desc">
          ...
          <g class="bus-map-route-halo">${[...outboundPaths, ...inboundPaths].map((path) => `<path d="${path}"/>`).join("")}</g>
          <g class="bus-map-route bus-map-route-outbound">
            ${outboundPaths.map((path, index) => `<path d="${path}"${index === outboundPaths.length - 1 ? ' marker-end="url(#bus-401-arrow-outbound)"' : ""}/>`).join("")}
          </g>
          <g class="bus-map-route bus-map-route-inbound">
            ${inboundPaths.map((path, index) => `<path d="${path}"${index === inboundPaths.length - 1 ? ' marker-end="url(#bus-401-arrow-inbound)"' : ""}/>`).join("")}
          </g>
    ...
    <section class="bus-page shell section-drawer">
      ...
      <div class="bus-numbering" role="tablist" aria-label="按编号范围查看公交线路">
        ${busNumbering.map((item, index) => `
          <button class="bus-number-tab${index === 0 ? " active" : ""}" type="button" role="tab" aria-selected="${index === 0}"
            aria-controls="bus-range-panel-${index}" data-bus-range="${index}">
            <span>${item.range}</span><small>${item.en}</small><h3>${item.title}</h3><p>${item.desc}</p><b aria-hidden="true">查看线路 →</b>
          </button>`).join("")}
      </div>
      <div class="bus-range-results" aria-live="polite">
        ${busNumbering.slice(0, 3).map((item, index) => `
        ...
        <section class="bus-range-panel" id="bus-range-panel-3" data-bus-panel="3" role="tabpanel" hidden>
          <div class="bus-route-directory">
            <div class="news-list-heading"><p>OUTER DISTRICT ROUTES</p><h2>郊区分区线路</h2></div>
            ...
            <a class="news-list-item" href="#transit-bus-401">
              ...
              <div><small>江洲区</small><h3>401路</h3><p>洲角广场—江洲北站</p></div><b aria-hidden="true">→</b>
            </a>
          </div>
        </section>
      </div>
...
function busRouteDetailTemplate(section, route) {
  return `
    ${sectionHeroTemplate(section, 1, { href: "#transit-bus", label: "公交" })}
    <section class="bus-page bus-route-detail-page shell section-drawer">
      <header class="bus-page-heading">
        <p>ZEZHOU CITY BUS · ROUTE ${route.number}</p>
        <h2>${route.number}路线路信息</h2>
      </header>
      ${busRouteCardTemplate(route)}
    </section>`;
}
...
async function render() {
  ...
  const busRouteDetail = slug === "transit-bus-301"
    ...
    : slug === "transit-bus-401"
      ? busRoute401
      : null;
  const transitSection = sections.find((item) => item.slug === "transit");
  ...
  const nextMarkup = busRouteDetail
    ? busRouteDetailTemplate(transitSection, busRouteDetail)
    : isNewsArticle
  ...
  const parentSection = busRouteDetail
    ? transitSection
    : isNewsArticle
    ? newsSection
    : newsCategory
  ...
  if (parentSection) {
    ...
    const backRoute = busRouteDetail
  ...
  document.title = busRouteDetail
    ? `${busRouteDetail.number}路｜泽州市门户`
    : isNewsArticle
    ? isTrafficNewsArticle
